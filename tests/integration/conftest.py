...
from custom_components.besen.const import DOMAIN, PLATFORMS
...
def mock_discovered_service_info(enable_bluetooth: None) -> Generator[Mock]:
    ...
    with patch(
        "custom_components.besen.config_flow.async_discovered_service_info",
...
def mock_besen_client() -> Generator[Mock]:
    ...
    with (
        patch("custom_components.besen.BesenClient", autospec=True) as mock_client,
        patch(
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "custom_components.besen.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
async def setup_integration(
    ...
    with patch(
        "custom_components.besen.PLATFORMS",
...
@pytest.fixture(autouse=True)
def enable_besen_custom_integration(enable_custom_integrations: None) -> None:
    """Load this repository integration instead of the built-in version."""


@pytest.fixture
def entity_registry_enabled_by_default() -> Generator[None]:
    """Enable diagnostic entities when asserting the complete Core platform."""
    with patch(
