...
| Dependency | `besen==0.4.2` | HACS 0.5.2 uses `besen==0.4.5` for start-charging response handling; the pinned Core baseline uses `0.4.2` |
| Packaging | HACS and library shared release numbering | HACS 0.5.2 and Python library 0.4.5 have independent releases |

## Necessary differences from Core

- HACS 0.5.2 and `main` use `besen==0.4.5` for start-charging response handling while the pinned Core baseline uses `0.4.2`. This exact dependency override is recorded in `core-baseline.json`; all other baseline checks remain enforced. Published HACS 0.5.0 keeps its original 0.4.2 dependency.
